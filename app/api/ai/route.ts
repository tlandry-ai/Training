...
async function generateText(client: Anthropic, prompt: string, maxTokens = 400) {
  ...
  try {
    switch (action) {
      case 'daily-brief': {
        ...
        const text = await generateText(client, prompt, 250)
      ...
      case 'skill-analysis': {
        ...
        const text = await generateText(client, prompt, 400)
        const json = extractJson(text)
        return NextResponse.json({
      ...
      case 'goal-nudge': {
        ...
        const text = await generateText(client, prompt, 200)
        return NextResponse.json({ text })
      }
