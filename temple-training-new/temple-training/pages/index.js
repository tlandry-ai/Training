...
export default function Home() {
  ...
  async function loadData() {
    setLoading(true)
    const [{ data:ci }, { data:no }, { data:go }] = await Promise.all([
    ...
    const ciMap = {}
    ;(ci||[]).forEach(r => {
      if (!ciMap[r.date_key]) ciMap[r.date_key] = {}
      ciMap[r.date_key][r.block_id] = r.done
    })
    const noMap = {}
    ;(no||[]).forEach(r => {
      if (!noMap[r.date_key]) noMap[r.date_key] = []
      noMap[r.date_key].push({ text:r.text, ts:new Date(r.created_at).toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'}) })
    })
    setCheckins(ciMap)
    setNotes(noMap)
    setGoals(go||[])
  ...
  async function saveGoal() {
    ...
    const { data } = await supabase.from('goals').insert({
      title:newGoal.title.trim(), cat:newGoal.cat,
      deadline:newGoal.deadline||null, note:newGoal.note||null, done:false
    }).select().single()
    if (data) setGoals(prev => [...prev, data])
    setNewGoal({ title:'', cat:'gym', deadline:'', note:'' })
    setAddingGoal(false)
  }
  ...
  return (
    <div style={{ background:C.bg, minHeight:'100vh', color:C.text, fontFamily:"'DM Sans', sans-serif" }}>
      ...
      <div style={{ background:C.surface, borderBottom:`1px solid ${C.border}`, padding:'20px 32px', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
        <div>
          <h1 style={{ fontFamily:"'DM Serif Display', serif", fontSize:'1.9rem', letterSpacing:'-0.5px', color:C.text, lineHeight:1 }}>
            Summer Training
          </h1>
      ...
      {loading && (
        <div style={{ padding:'60px', textAlign:'center', fontFamily:"'DM Mono', monospace", fontSize:'0.7rem', color:C.muted, letterSpacing:'0.08em', textTransform:'uppercase' }}>
          Loading...
        </div>
      )}
            ...
            <>
              ...
              <div style={{ display:'flex', flexDirection:'column', gap:6, marginBottom:36 }}>
                {todayPlan.blocks.map(block => {
                  ...
                  return (
                    <div key={block.id} className="brow" onClick={() => toggleCheckin(today, block.id)} style={{
                      display:'flex', alignItems:'center', gap:14,
                      background: isDone ? block.color.bg : C.surface,
                      border: `1px solid ${isDone ? block.color.border : C.border}`,
                      borderRadius:8, padding:'13px 16px',
                    }}>
                      <div style={{
                        width:20, height:20, borderRadius:'50%', flexShrink:0,
                        border: `1.5px solid ${isDone ? block.color.text : C.border}`,
                        background: isDone ? block.color.text : 'transparent',
                      ...
                      {isDone && (
                        <span style={{ fontSize:'0.58rem', fontFamily:"'DM Mono', monospace", padding:'2px 8px', borderRadius:10, background:block.color.text+'22', color:block.color.text, border:`1px solid ${block.color.border}`, letterSpacing:'0.04em' }}>
                          DONE
                        </span>
                      )}
                    </div>
              ...
              <div>
                ...
                <div style={{ display:'flex', gap:8 }}>
                  <input
                    ...
                    style={{ flex:1, background:C.surface, border:`1px solid ${C.border}`, borderRadius:8, padding:'11px 14px', color:C.text, fontSize:'0.85rem', fontFamily:"'DM Sans', sans-serif" }}
                  />
                  <button onClick={saveNote} disabled={saving} style={{ background:C.text, border:'none', borderRadius:8, padding:'11px 18px', color:C.bg, fontFamily:"'DM Mono', monospace", fontSize:'0.65rem', letterSpacing:'0.06em', textTransform:'uppercase', cursor:'pointer' }}>
                    {saving ? '...' : 'Save'}
                  </button>
                </div>
              </div>
      ...
      {!loading && tab === 'calendar' && (
        <div style={{ padding:'32px' }}>
          <div style={{ display:'flex', gap:14, flexWrap:'wrap', marginBottom:28 }}>
            {[
              { label:'Solid Core', color:C.solid },
              { label:'CorePower',  color:C.cp    },
              { label:'Lift',       color:C.lift  },
              { label:'Practice',   color:C.gym   },
              { label:'Work',       color:C.work  },
                  ...
                  {cells.map((date, i) => {
                    ...
                    return (
                      <div key={i} className="cal-day-cell" style={{
                        background: plan.isWeekend ? C.borderLight : plan.isActive ? C.surface : '#fafaf7',
                        border: `1px solid ${isToday ? C.text : C.border}`,
                        borderRadius:6, padding:'6px 7px', minHeight:88,
                        opacity: plan.isWeekend ? 0.5 : 1,
                        ...
                        {plan.blocks.map(block => {
                          const isDone = !!dc[block.id]
                          return (
                            <div key={block.id} style={{
                              display:'block', fontSize:'0.52rem', fontWeight:600,
                              padding:'1px 4px', borderRadius:2, marginBottom:2,
      ...
      {!loading && tab === 'goals' && (
        <div style={{ padding:'32px', maxWidth:660 }}>
          <div style={{ display:'flex', gap:12, alignItems:'center', marginBottom:28 }}>
            ...
            <div style={{ fontFamily:"'DM Mono', monospace", fontSize:'0.65rem', color:C.muted, whiteSpace:'nowrap', textTransform:'uppercase', letterSpacing:'0.05em' }}>
          ...
          <div style={{ display:'flex', gap:6, flexWrap:'wrap', marginBottom:20 }}>
            {[{ id:'all', label:'All' }, ...CATS].map(f => (
              <button key={f.id} onClick={() => setFilterCat(f.id)} style={{
                background: filterCat===f.id ? C.text : 'transparent',
                border: `1.5px solid ${filterCat===f.id ? C.text : C.border}`,
                borderRadius:4, padding:'4px 12px', cursor:'pointer',
                fontFamily:"'DM Mono', monospace", fontSize:'0.6rem', letterSpacing:'0.06em', textTransform:'uppercase',
                color: filterCat===f.id ? C.bg : C.muted, transition:'all 0.12s',
              }}>{f.label}</button>
            ))}
          </div>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:6, marginBottom:20 }}>
            {filteredGoals.length === 0 && (
              <div style={{ fontFamily:"'DM Mono', monospace", fontSize:'0.68rem', color:C.muted, padding:'24px 0', textTransform:'uppercase', letterSpacing:'0.05em' }}>
            ...
            {filteredGoals.map(goal => {
              ...
              return (
                <div key={goal.id} className="goal-row brow" onClick={() => toggleGoal(goal.id)} style={{
                  display:'flex', alignItems:'flex-start', gap:14,
                  background: goal.done ? '#eef5ee' : C.surface,
                  border: `1px solid ${goal.done ? '#b8d4b5' : C.border}`,
                  borderRadius:8, padding:'13px 16px', cursor:'pointer', position:'relative',
                }}>
                  <div style={{
                    marginTop:2, width:18, height:18, borderRadius:'50%', flexShrink:0,
                    border: `1.5px solid ${goal.done ? '#2d5a28' : C.border}`,
                    background: goal.done ? '#2d5a28' : 'transparent',
                    display:'flex', alignItems:'center', justifyContent:'center', transition:'all 0.15s',
                  }}>
                  ...
                  <div style={{ flex:1, minWidth:0 }}>
                    <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:2, flexWrap:'wrap' }}>
                      <div style={{ fontWeight:600, fontSize:'0.88rem', color:goal.done ? C.muted : C.text, textDecoration:goal.done ? 'line-through' : 'none' }}>
                        {goal.title}
                      </div>
                      <span style={{ fontSize:'0.58rem', fontFamily:"'DM Mono', monospace", padding:'1px 7px', borderRadius:10, background:cat.color+'18', color:cat.color, border:`1px solid ${cat.color}33`, letterSpacing:'0.04em', textTransform:'uppercase' }}>
                        {cat.label}
                      </span>
                    </div>
                  ...
                  <button className="del-btn" onClick={e => { e.stopPropagation(); deleteGoal(goal.id) }} style={{
                    opacity:0, background:'none', border:'none', color:C.muted, cursor:'pointer', fontSize:'0.85rem', padding:'0 4px', flexShrink:0, transition:'opacity 0.15s',
                  }}>✕</button>
                </div>
              )
            })}
          ...
          {!addingGoal ? (
            <button className="add-btn" onClick={() => setAddingGoal(true)} style={{
              width:'100%', background:'transparent', border:`1.5px dashed ${C.border}`,
              borderRadius:8, padding:'13px', color:C.muted,
            ...
            <div style={{ background:C.surface, border:`1px solid ${C.border}`, borderRadius:8, padding:'20px' }}>
              ...
              <input value={newGoal.title} onChange={e => setNewGoal(p => ({...p, title:e.target.value}))} placeholder="Goal title..."
                style={{ width:'100%', background:C.bg, border:`1px solid ${C.border}`, borderRadius:6, padding:'10px 14px', color:C.text, fontSize:'0.88rem', fontFamily:"'DM Sans', sans-serif", marginBottom:10 }}
              />
              <div style={{ display:'flex', gap:8, marginBottom:10 }}>
                <select value={newGoal.cat} onChange={e => setNewGoal(p => ({...p, cat:e.target.value}))} style={{ flex:1, background:C.bg, border:`1px solid ${C.border}`, borderRadius:6, padding:'10px 14px', color:C.text, fontSize:'0.82rem', fontFamily:"'DM Sans', sans-serif" }}>
                  {CATS.map(cc => <option key={cc.id} value={cc.id}>{cc.label}</option>)}
                </select>
                <input value={newGoal.deadline} onChange={e => setNewGoal(p => ({...p, deadline:e.target.value}))} placeholder="Target (e.g. Aug 5)"
                  style={{ flex:1, background:C.bg, border:`1px solid ${C.border}`, borderRadius:6, padding:'10px 14px', color:C.text, fontSize:'0.82rem', fontFamily:"'DM Sans', sans-serif" }}
                />
              </div>
              <input value={newGoal.note} onChange={e => setNewGoal(p => ({...p, note:e.target.value}))} placeholder="Notes or progression plan (optional)..."
                style={{ width:'100%', background:C.bg, border:`1px solid ${C.border}`, borderRadius:6, padding:'10px 14px', color:C.text, fontSize:'0.82rem', fontFamily:"'DM Sans', sans-serif", marginBottom:14 }}
              />
              <div style={{ display:'flex', gap:8 }}>
                <button onClick={saveGoal} style={{ background:C.text, border:'none', borderRadius:6, padding:'10px 20px', color:C.bg, fontFamily:"'DM Mono', monospace", fontSize:'0.65rem', letterSpacing:'0.06em', textTransform:'uppercase', cursor:'pointer' }}>Save</button>
                <button onClick={() => setAddingGoal(false)} style={{ background:'transparent', border:`1px solid ${C.border}`, borderRadius:6, padding:'10px 20px', color:C.muted, fontFamily:"'DM Mono', monospace", fontSize:'0.65rem', letterSpacing:'0.06em', textTransform:'uppercase', cursor:'pointer' }}>Cancel</button>
              </div>
            </div>
          )}
        </div>
